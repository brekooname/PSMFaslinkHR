    ...
    public class XuLyDuLieu_1dong : LogicBase
    {
        ...
        public void doAnalyza(DateTime tuNgay, DateTime denNgay, string empID, string depID, int group1ID, bool chkLamTron)
        {
            ...
            try
            {
                ...
                var chukyluong = new DateTime(denNgay.Year, denNgay.Month, 1); // Để tính bảo hiểm.
                ...
                Provider.LoadData(ds, ds.tbCachTinhThueTNCN.TableName);
                lp.OutMessage("Công thức tính Thuế TNCN: " + ds.tbCachTinhThueTNCN.Rows.Count);
                Provider.LoadData(ds, ds.tblEmpPIT_Family.TableName);
                lp.OutMessage("Số người phụ thuộc: " + ds.tblEmpPIT_Family.Rows.Count);
                ...
                foreach (string eID in lstEmp)
                {
                    ...
                    try
                    {
                        ...
                        if (e1["ContractDate"] != DBNull.Value && e1.ContractDate <= denNgay)
                        {
                            var listEmpUnpaySIHI = ds.tblEmpUnpayable_SIHI.Where(p => p.EmployeeID == eID);
                            if (listEmpUnpaySIHI.Count() >= 1)
                            {
                                var empUnpaySIHI = listEmpUnpaySIHI.Where(p => Convert.ToDateTime(p.From_MY) <= chukyluong && Convert.ToDateTime(p.To_MY) >= chukyluong).FirstOrDefault();
                                if (empUnpaySIHI == null)
                                {
                                    isTinhBH = true;
                                }
                            }
                            ...
                            if (e1["LeftDate"] != DBNull.Value && e1.LeftDate >= tuNgay && e1.LeftDate < denNgay)
                            {
                                isTinhBH = false;
                            }
                        }
                        ...
                        if (isTinhBH)
                        {
                            var soTienTinhBH = (r.luongCB_Moi + r.PhuCapTrachNhiem) >= 23000000 ? 23000000 : (r.luongCB_Moi + r.PhuCapTrachNhiem);
                            r.tienBHXH = soTienTinhBH * 8 / 100;
                            r.tienBHYT = soTienTinhBH * 1.5 / 100;
                            r.tienBHTN = soTienTinhBH * 1 / 100;
                        ...
                        var soNguoiPhuThuoc = ds.tblEmpPIT_Family.Where(p => p.EmployeeID == eID).OrderByDescending(p => p.DateChange).FirstOrDefault();
                        if (soNguoiPhuThuoc != null)
                        {
                            r.SoNguoiPhuThuoc = soNguoiPhuThuoc.NumPerson;
                        }
                        else
                            r.SoNguoiPhuThuoc = 0;
                        r.LuongThucNhanTinhThue = r.tongLuong - (r.tongLuong > 0 ? r.tongKhauTru : 0); // Nếu thực nhận tính thuế < 9tr. Không phải tính.
                        if (r.LuongThucNhanTinhThue > 9000000)
                        {
                            r.ThueTNCN = TinhThueTNCN(r.LuongThucNhanTinhThue, r.SoNguoiPhuThuoc, ds.tbCachTinhThueTNCN);
                        }
                        #endregion
        ...
        private double TinhThueTNCN(double LuongThucNhanTinhThue, int soNguoiPhuThuoc,dsTinhLuong.tbCachTinhThueTNCNDataTable dt)
        {
            double totalTNCN = 0;
            ...
            var calc = dt.Where(p=>p.SoTienBatDau <= thunhapTinhThue && thunhapTinhThue < p.SoTienDen).FirstOrDefault();
            if (calc != null)
            {
                totalTNCN = thunhapTinhThue * calc.ThueSuat / 100 + calc.SoTienBuTru;
            }
