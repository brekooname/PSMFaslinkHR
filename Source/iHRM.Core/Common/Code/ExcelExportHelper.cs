...
namespace iHRM.Common.Code
{
    ...
    public class baseExcelExportHelper : ExcelExtend
    {
        ...
        public void FillDataTable_DSNL(DataTable dt)
        {
            int FirstRow, FirstColumn;
            var lst = GetBC_FillDataHead(out FirstRow, out FirstColumn);
            FillDataTableByHead_DSNL(dt, FirstRow, FirstColumn, lst);

        }

        public void FillDataTable_XuatBH(DataTable dt)
        {
            int FirstRow, FirstColumn;
            var lst = GetBC_FillDataHead(out FirstRow, out FirstColumn);
            FillDataTableByHead_XuatBH(dt, FirstRow, FirstColumn, lst);
                        ...
                        else if (col.StartsWith("#"))
                        {
                            ...
                            if (dt.Columns.Contains(col))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (KTSo(value.ToString()) && isSo == false && value.ToString() != "")
                                    {
                                        value = string.Format("{0:#,0.##}", value);
                                        //value = Convert.ToDouble(value);
                                        isSo = true;
                                ...
                                if (!(value == DBNull.Value || value == null))
                                {
                                    if (isSo)
                                    {
                                        if (value.ToString() == "")
                                        {
                                            WriteToCell(rowIdx + i, colIdx + j, Convert.ToDouble(0));
                                        }
                                        else
                                        {
                                            WriteToCell(rowIdx + i, colIdx + j, Convert.ToDouble(value));
                                        }    
                                    }
                                    else
                                    {
                                        if(value.ToString().StartsWith("PT"))
                                        {
                                            WriteToCell(rowIdx + i, colIdx + j, value.ToString().Replace("PT",""),true);
                                        }
                                        else
                                        {
                                            WriteToCell(rowIdx + i, colIdx + j, value.ToString());
                                        }
                                    }
                                }
                            }
        ...
        public void FillDataTableByHead_DSNL(DataTable dt, int rowIdx, int colIdx, List<string> headDataIndex)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                base.InsertRow(rowIdx + i + 1);
                    ...
                    else if (col.StartsWith("#"))
                    {
                        ...
                        if (dt.Columns.Contains(col))
                        {
                            ...
                            else
                            {
                                ...
                                else if ((value is float || value is double || value is decimal) && value.ToString().Length > 1 )
                                {
                                    value = string.Format("{0:#,0.##}", value);
                                    isSo = true;
                                }
                            ...
                            if (!(value == DBNull.Value || value == null))
                            {
                                if (isSo)
                                {
                                    if (value.ToString() == "")
                                    {
                                        WriteToCell(rowIdx + i, colIdx + j, Convert.ToDecimal(0));
                                    }
                                    else
                                    {
                                        WriteToCell(rowIdx + i, colIdx + j, Convert.ToDecimal(value));
                                    }
                                }
                                else
                                {
        ...
        public void FillDataTableByHead_XuatBH(DataTable dt, int rowIdx, int colIdx, List<string> headDataIndex)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                base.InsertRow(rowIdx + i + 1);
        ...
        public bool KTSo(string pValue) // jun add 17/05/2018 đề kiểm tra số <2
        {
        ...
        public void FillDataTableByHead<T>(IList<T> dt, int rowIdx, int colIdx, List<string> headDataIndex) // Cái này không sài
        {
                    ...
                    else if (col.StartsWith("#"))
                    {
                        ...
                        if (propertyInfo != null)
                        {
                            ...
                            if (value is bool)
                                ...
                                outputvalue = string.Format("{0}", value.ToString());
