...
{
    public partial class frmDinhNghiaThamSo_Fake : frmBase
    {
        dcLuongv3DataContext db;
        List<tbLuong_DinhNghiaThamSo_Fake> LstData;
        ...
        public frmDinhNghiaThamSo_Fake()
        {
            InitializeComponent();
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.Caption = "Đang tải dữ liệu tham số tính lương báo cáo...";
            dw_it.OnDoing = (s,ev) => 
            {
                if (string.IsNullOrWhiteSpace(txtSearchKey.Text))
                    LstData = db.tbLuong_DinhNghiaThamSo_Fakes.ToList();
                else
                    LstData = db.tbLuong_DinhNghiaThamSo_Fakes.Where(i => i.ten.IndexOf(txtSearchKey.Text.Trim()) >= 0).ToList();
                dw_it.bw.ReportProgress(1, LstData);
            };
            dw_it.OnProcessing = (ps, data) =>
            {
                grd.DataSource = new BindingList<tbLuong_DinhNghiaThamSo_Fake>(LstData);
        ...
        {
            var drs = grv.GetSelectedRows().Select(i => grv.GetRow(i) as tbLuong_DinhNghiaThamSo_Fake);
            if (drs.Count() == 0)
            {
            ...
            try
            {
                db.tbLuong_DinhNghiaThamSo_Fakes.DeleteAllOnSubmit(drs);
                db.SubmitChanges();
                ...
                var log = Log2.CreateLog(Log2.Log2Action.xoa, "Xóa " + drs.Count() + " bản ghi tham số lương báo cáo", "tbLuong_DinhNghiaThamSo_Fake");
        ...
        {
            var r = grv.GetFocusedRow() as tbLuong_DinhNghiaThamSo_Fake;
            if (r != null)
                show_edit(r);
        }
        void show_edit(tbLuong_DinhNghiaThamSo_Fake r)
        {
            dlgEditor.MyValue = r;
            ...
            if (dlgEditor.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    var log = Log2.CreateLog(Log2.Log2Action.sua, "Sửa tham số lương báo cáo " + r.ten, "tbLuong_DinhNghiaThamSo_Fake");
        ...
        {
            dlgEditor.MyValue = new tbLuong_DinhNghiaThamSo_Fake();
                ...
                try
                {
                    if (string.IsNullOrWhiteSpace(dlgEditor.MyValue.tsIdx))
                    {
                        ...
                        if (dlgEditor.MyValue.kieuDuLieu == 2)
                            ts_Pre = "tss";
                        for (i = 1; i < 199; i++)
                            if (db.tbLuong_DinhNghiaThamSo_Fakes.Count(j => j.tsIdx == ts_Pre + i) == 0)
                                break;
                        if (i >= 199)
                        {
                    ...
                    db.tbLuong_DinhNghiaThamSo_Fakes.InsertOnSubmit(dlgEditor.MyValue);
                    db.SubmitChanges();
                    ...
                    var log = Log2.CreateLog(Log2.Log2Action.them, "Thêm tham số lương báo cáo " + dlgEditor.MyValue.ten, "tbLuong_DinhNghiaThamSo_Fake");
                    Log2.AddLogDetail(log, new string[] {"id",
                        "ma",
                        "ten",
                        "tsIdx",
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            if (r != null)
            {
                viewThamSoCongThuc_Fake v = new viewThamSoCongThuc_Fake();
        ...
        private void frmDinhNghiaThamSo_Fake_KeyDown(object sender, KeyEventArgs e)
        {
