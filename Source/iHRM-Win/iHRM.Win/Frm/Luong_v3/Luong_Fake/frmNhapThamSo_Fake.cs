...
{
    public partial class frmNhapThamSo_Fake : frmBase
    {
        ...
        dlgChuyenBangLuong_Fake dlg = new dlgChuyenBangLuong_Fake();

        i_Import.FixImporter importer = null;

        public frmNhapThamSo_Fake()
        {
            InitializeComponent();
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.OnDoing = (s,ev) => 
            {
                var lst1 = db.tbLuong_DinhNghiaThamSo_Fakes.Where(i => i.kieuNhap == 1).ToList();
                dw_it.bw.ReportProgress(1, lst1);
                
                LstData = Provider.ExecuteDataTableReader("p_luong_nhapTS_Fake", 
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsLock.IsLock.Check_IsLock("tbLuong_NhapDuLieu_Fake", txtSearchThang.DateTime))
                {
                ...
                {
                    var log = Log2.CreateLog(Log2.Log2Action.sua, "Nhập dữ liệu bảng lương báo cáo nhân viên " + DbHelper.DrGet(r, "idNV") + " tháng " + DbHelper.DrGet(r, "thang") + " id " + DbHelper.DrGet(r, "id"), "tbLuong_NhapDuLieu_Fake", DbHelper.DrGet(r, "idNV").ToString());
                ...
                Provider.UpdateData(LstData, "tbLuong_NhapDuLieu_Fake");
        ...
        private void importer_OnDownLoadFileTemplate(string fPath)
        {
            ...
            var lst1 = db.tbLuong_DinhNghiaThamSo_Fakes.Where(i => i.kieuNhap == 1).OrderBy(p => p.ten).ToList();
            ...
            //in dữ liệu có sẵn
            var ret = Provider.ExecuteDataTableReader("p_Luong_NhapTSLuong_GetAll_Fake",
                new SqlParameter("thang", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)),
                new SqlParameter("phongban_id", chonPhongBan1.SelectedValue)
            );
        ...
        private void Importer_OnImportData(DataTable obj)
        {
            try
            {
                if (IsLock.IsLock.Check_IsLock("tbLuong_NhapDuLieu_Fake", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)))
                {
                    GUIHelper.MessageBox("Dữ liệu đã chốt khổng thể thao tác!");
                    return;
                }
                ...
                var ret = Provider.ExecNoneQuery("p_Luong_NhapTSLuong_UpdateMulti_Fake", 
                    new SqlParameter("thang", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)),
                    new SqlParameter("data", SqlDbType.Structured) { TypeName = "LST_nhapTsLuong", Value = dt }
                );
                foreach (DataRow r in dt.Rows)
                {
                    var log = Log2.CreateLog(Log2.Log2Action.them, "Nhập dữ liệu bảng lương báo cáo từ excel " + DbHelper.DrGet(r, "nhanvien_ma") + " tháng " + DbHelper.DrGet(r, "thang"), "tbLuong_NhapDuLieu_Fake");
                ...
                if (dg == DialogResult.OK)
                {
                     ...
                     if (dg2 == DialogResult.OK)
                     {
                        var thang = new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1);
                         Provider.ExecNoneQuery("p_NhapTS_deleteTS_Fake",
                            new SqlParameter("thang", thang),
                            new SqlParameter("phongban_id", chonPhongBan1.SelectedValue)
                        );
                         GUIHelper.Notifications_msg(GUIHelper.Notifications_msgType.DelSuccess);

                        var log = Log2.CreateLog(Log2.Log2Action.phantich, "Xóa dữ liệu nhập lương báo cáo theo phòng ban " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLie_Fake");
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var thang = new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1);
                    Provider.ExecNoneQuery("p_NhapTS_checkCreate_Fake", new SqlParameter("thang", thang));
                    GUIHelper.Notifications_msg(GUIHelper.Notifications_msgType.AddSuccess);

                    var log = Log2.CreateLog(Log2.Log2Action.phantich, "Tải thêm danh sách nhân viên bảng lương báo cáo " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLieu_Fake");
                    log.tbLog2_details.Add(new Core.Business.DbObject.tbLog2_detail()
                    {
                        id = Guid.NewGuid(),
                        log_id = log.id,
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dg == DialogResult.OK)
                {
                    ...
                    var log = Log2.CreateLog(Log2.Log2Action.phantich, "Khởi tạo dữ liệu lương báo cáo đầu kỳ tháng " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLieu_Fake");
                    log.tbLog2_details.Add(new Core.Business.DbObject.tbLog2_detail()
                    {
                        id = Guid.NewGuid(),
                        log_id = log.id,
        ...
        {
            dlg = new dlgChuyenBangLuong_Fake();
            dlg.ShowDialog();
        }
        ...
        #endregion
        private void frmNhapThamSo_Fake_KeyDown(object sender, KeyEventArgs e)
        {
