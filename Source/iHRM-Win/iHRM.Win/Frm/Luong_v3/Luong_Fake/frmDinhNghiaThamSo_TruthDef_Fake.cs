...
{
    public partial class frmDinhNghiaThamSo_TruthDef_Fake : frmBase
    {
        dcLuongv3DataContext db = new dcLuongv3DataContext(Core.Business.Provider.ConnectionString);
        IQueryable<tbLuongTS_TruthDef_Fake> LstData;

        tbLuong_DinhNghiaThamSo_Fake myTS = new tbLuong_DinhNghiaThamSo_Fake();
        ...
        public int? MyId
        {
            ...
            {
                myId = value;
                myTS = db.tbLuong_DinhNghiaThamSo_Fakes.SingleOrDefault(i => i.id == myId);
                LstData = db.tbLuongTS_TruthDef_Fakes.Where(i => i.luongTS_id == myId);
                grd.DataSource = LstData;
            }
        }

        public frmDinhNghiaThamSo_TruthDef_Fake()
        {
            InitializeComponent();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var log = Log2.CreateLog(Log2.Log2Action.xoa, "Xóa " + db.GetChangeSet().Deletes.Count() + " bản ghi", "tbLuongTS_TruthDef_Fake");
                    log.tbLog2_details.AddRange(db.GetChangeSet().Deletes.Select(i => new Core.Business.DbObject.tbLog2_detail()
                    {
                        id = Guid.NewGuid(),
                        log_id = log.id,
                        target1 = "id",
                        value1 = (i as tbLuongTS_TruthDef_Fake).id.ToString()
                    }).ToArray());
                    Log2.PushLog(log);
                }
                ...
                {
                    foreach (tbLuongTS_TruthDef_Fake r in db.GetChangeSet().Inserts)
                    {
                        var log = Log2.CreateLog(Log2.Log2Action.them, "Thêm bản ghi " + r.id, "tbLuongTS_TruthDef_Fake");
                ...
                {
                    foreach (tbLuongTS_TruthDef_Fake r in db.GetChangeSet().Updates)
                    {
                        var log = Log2.CreateLog(Log2.Log2Action.sua, "Sửa bản ghi " + r.id, "tbLuongTS_TruthDef_Fake");
                        Log2.AddLogDetail(log, new string[] {"id",
                            "luongTS_id",
                            "tuGiaTri",
                            "tuGiaTris",
        ...
        {
            var r = grv.GetRow(e.RowHandle) as tbLuongTS_TruthDef_Fake;
            if (r != null)
            {
                r.id = Guid.NewGuid();
                r.luongTS_id = myId;
