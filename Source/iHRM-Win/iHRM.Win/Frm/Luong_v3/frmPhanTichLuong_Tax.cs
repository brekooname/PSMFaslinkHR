...
{
    public partial class frmPhanTichLuong_Tax : frmBase
    {
        dcLuongv3DataContext db;

        public frmPhanTichLuong_Tax()
        {
            InitializeComponent();
        }
        ...
        {
            btnExxecute.Enabled = checkPhanTichOK.Checked;
        }
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                var ds = Provider.ExecuteDataSetReader("p_luong_layDuLieuTinhLuong_Tax",
                ...
                if (chkDlNhapTay.Checked)
                {
                    try
                    {
                        ...
                        sql += @"UPDATE dbo.tbLuong_BangLuongThang_Tax
                        SET " + string.Join(",", lst.Select(i => string.Format("[{0}]=n.[{0}]", i))) + @"
                        FROM dbo.tbLuong_BangLuongThang_Tax b  
	                        INNER JOIN dbo.tbLuong_NhapDuLieu_Tax n ON b.idNV = n.idNV AND b.thang = n.thang
	                        LEFT JOIN dbo.tblEmployee nv ON n.idNV=nv.EmployeeID 
...
//                                                    FROM dbo.tbLuong_BangLuongThang b  
//	                                                    " + (string.IsNullOrWhiteSpace(tenBang) ? "" : ( //neeus ko dien ten bang thi thoi ko join nua //ducnm 16/8
//                                                               "INNER JOIN [" + tenBang + @"] n ON b.idNV = n.[" + tenCotNvId + @"] " + (string.IsNullOrWhiteSpace(tenCotThang) ? "" : (" AND b.thang = n.[" + tenCotThang + @"] "))
//                                                             )) + @"
//	                                                    LEFT JOIN dbo.tblEmployee nv ON b.idNV = nv.EmployeeID
//	                                                    LEFT JOIN dbo.tbNhanVien_NhomNV nn ON nn.employee_id = nv.EmployeeID
//                                                    WHERE " + where;
...
//                                        FROM dbo.tbLuong_BangLuongThang b  
//	                                        INNER JOIN [tbMod_values] n ON b.idNV = n.[nhanvien_id] AND b.thang = n.[thang]
//	                                        LEFT JOIN dbo.tblEmployee nv ON n.[nhanvien_id] = nv.EmployeeID
//	                                        LEFT JOIN dbo.tbNhanVien_NhomNV nn ON nn.employee_id = nv.EmployeeID
//                                        WHERE " + where;
//                                             Provider.ExecuteNonQuery_SQL(sql, new SqlParameter("thang", txtSearchThang.DateTime));
//                                             break;
...
//                }
                            ...
                            try
                            {
                                ...
                                if (DbHelper.DrGetInt(r, "kieuMapping") == 2) //được định nghĩa
                                {
                                    ...
                                    if (DbHelper.DrGetInt(r, "kieuNhap") == 1) //nhập tay
                                    {
                                        ...
                                        FROM dbo.tbLuong_BangLuongThang_Tax b
	                                        INNER JOIN dbo.tbLuong_NhapDuLieu_Tax nl ON nl.idNV = b.idNV AND nl.thang = b.thang
	                                        INNER JOIN tbLuongTS_TruthDef_Tax t ON nl.[" + DbHelper.DrGetString(r, "tsIdx") + @"] = t." + ts_pre + " AND t.luongTS_id = " + DbHelper.DrGetString(r, "id") + @"
	                                        LEFT JOIN dbo.tblEmployee nv ON nl.idNV=nv.EmployeeID
                                       INNER JOIN ( SELECT * FROM dbo.tblRef_Department d  WHERE d.[Path] LIKE  @DepPath) d1 ON nv.DepID_Final = d1.DepID
                                        WHERE " + where;
                                        }else
                                        {
                                        ...
                                        FROM dbo.tbLuong_BangLuongThang_Tax b
	                                        INNER JOIN dbo.tbLuong_NhapDuLieu_Tax nl ON nl.idNV = b.idNV AND nl.thang = b.thang
	                                        INNER JOIN tbLuongTS_TruthDef_Tax t ON nl.[" + DbHelper.DrGetString(r, "tsIdx") + @"] = t." + ts_pre + " AND nl.[" + DbHelper.DrGetString(r, "tsIdx") + @"] < t.denGiaTri AND t.luongTS_id = " + DbHelper.DrGetString(r, "id") + @"
	                                        LEFT JOIN dbo.tblEmployee nv ON nl.idNV=nv.EmployeeID
                                        INNER JOIN ( SELECT * FROM dbo.tblRef_Department d  WHERE d.[Path] LIKE  @DepPath) d1 ON nv.DepID_Final = d1.DepID
                                        WHERE " + where;
                                        }
                                    ...
                                    else //tính toán hoặc hút dl
                                    {
                                        ...
                                        SET [" + DbHelper.DrGetString(r, "tsIdx") + @"] = CAST( t.layGiaTri AS Decimal ) 
                                        FROM dbo.tbLuong_BangLuongThang_Tax b
	                                        INNER JOIN tbLuongTS_TruthDef_Tax t ON b.[" + DbHelper.DrGetString(r, "tsIdx") + @"] = t." + ts_pre + " AND t.luongTS_id = " + DbHelper.DrGetString(r, "id") + @"
	                                        LEFT JOIN dbo.tblEmployee nv ON b.idNV=nv.EmployeeID
                                       INNER JOIN ( SELECT * FROM dbo.tblRef_Department d  WHERE d.[Path] LIKE  @DepPath) d1 ON nv.DepID_Final = d1.DepID
                                        WHERE " + where;
                                        }
                                        ...
                                        SET [" + DbHelper.DrGetString(r, "tsIdx") + @"] = t.layGiaTri
                                        FROM dbo.tbLuong_BangLuongThang_tax b
	                                        INNER JOIN tbLuongTS_TruthDef_Tax t ON b.[" + DbHelper.DrGetString(r, "tsIdx") + @"] >= t." + ts_pre + " AND b.[" + DbHelper.DrGetString(r, "tsIdx") + @"] < t.denGiaTri AND t.luongTS_id = " + DbHelper.DrGetString(r, "id") + @"
	                                        LEFT JOIN dbo.tblEmployee nv ON b.idNV=nv.EmployeeID
                                        INNER JOIN ( SELECT * FROM dbo.tblRef_Department d  WHERE d.[Path] LIKE  @DepPath) d1 ON nv.DepID_Final = d1.DepID
                                        WHERE " + where;
                                        }
                ...
                var log = Log2.CreateLog(Log2.Log2Action.phantich, "Phân tích lương lương tháng: " + string.Format("{0:MM/yyyy}", txtSearchThang.DateTime), "tbCongThucTinh_Tax");
        ...
        #endregion
        private void frmPhanTichLuong_Tax_KeyDown(object sender, KeyEventArgs e)
        {
