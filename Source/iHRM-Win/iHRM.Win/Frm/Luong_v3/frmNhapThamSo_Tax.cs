...
{
    public partial class frmNhapThamSo_Tax : frmBase
    {
        ...
        dlgChuyenBangLuong_Thue dlg = new dlgChuyenBangLuong_Thue();
        dlgChuyenBangLuongBaoCao_Thue dlg_BC = new dlgChuyenBangLuongBaoCao_Thue();

        i_Import.FixImporter importer = null;

        public frmNhapThamSo_Tax()
        {
            InitializeComponent();
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.OnDoing = (s,ev) => 
            {
                var lst1 = db.tbLuong_DinhNghiaThamSo_Taxes.Where(i => i.kieuNhap == 1).ToList();
                dw_it.bw.ReportProgress(1, lst1);
                
                LstData = Provider.ExecuteDataTableReader("p_luong_nhapTS_Tax", 
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsLock.IsLock.Check_IsLock("tbLuong_NhapDuLieu_Tax", txtSearchThang.DateTime))
                {
                ...
                {
                    var log = Log2.CreateLog(Log2.Log2Action.sua, "Nhập dữ liệu bảng lương nhân viên " + DbHelper.DrGet(r, "idNV") + " tháng " + DbHelper.DrGet(r, "thang") + " id " + DbHelper.DrGet(r, "id"), "tbLuong_NhapDuLieu_Tax", DbHelper.DrGet(r, "idNV").ToString());
                ...
                Provider.UpdateData(LstData, "tbLuong_NhapDuLieu_Tax");
        ...
        private void importer_OnDownLoadFileTemplate(string fPath)
        {
            ...
            var lst1 = db.tbLuong_DinhNghiaThamSo_Taxes.Where(i => i.kieuNhap == 1).OrderBy(p => p.ten).ToList();
            ...
            //in dữ liệu có sẵn
            var ret = Provider.ExecuteDataTableReader("p_Luong_NhapTSLuong_GetAll_Tax",
                new SqlParameter("thang", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)),
                new SqlParameter("phongban_id", chonPhongBan1.SelectedValue)
            );
        ...
        private void Importer_OnImportData(DataTable obj)
        {
            try
            {
                if (IsLock.IsLock.Check_IsLock("tbLuong_NhapDuLieu_Tax", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)))
                {
                    GUIHelper.MessageBox("Dữ liệu đã chốt khổng thể thao tác!");
                    return;
                }
                ...
                var ret = Provider.ExecNoneQuery("p_Luong_NhapTSLuong_UpdateMulti_Tax", 
                    new SqlParameter("thang", new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1)),
                    new SqlParameter("data", SqlDbType.Structured) { TypeName = "LST_nhapTsLuong", Value = dt }
                );
                foreach (DataRow r in dt.Rows)
                {
                    var log = Log2.CreateLog(Log2.Log2Action.them, "Nhập dữ liệu bảng lương từ excel " + DbHelper.DrGet(r, "nhanvien_ma") + " tháng " + DbHelper.DrGet(r, "thang"), "tbLuong_NhapDuLieu_Tax");
                ...
                if (dg == DialogResult.OK)
                {
                     ...
                     if (dg2 == DialogResult.OK)
                     {
                        var thang = new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1);
                         Provider.ExecNoneQuery("p_NhapTS_deleteTS_Tax",
                            new SqlParameter("thang", thang),
                            new SqlParameter("phongban_id", chonPhongBan1.SelectedValue)
                        );
                         GUIHelper.Notifications_msg(GUIHelper.Notifications_msgType.DelSuccess);

                        var log = Log2.CreateLog(Log2.Log2Action.phantich, "Xóa dữ liệu nhập theo phòng ban " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLie_Tax");
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var thang = new DateTime(txtSearchThang.DateTime.Year, txtSearchThang.DateTime.Month, 1);
                    Provider.ExecNoneQuery("p_NhapTS_checkCreate_Tax", new SqlParameter("thang", thang));
                    GUIHelper.Notifications_msg(GUIHelper.Notifications_msgType.AddSuccess);

                    var log = Log2.CreateLog(Log2.Log2Action.phantich, "Tải thêm danh sách nhân viên " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLieu_Tax");
                    log.tbLog2_details.Add(new Core.Business.DbObject.tbLog2_detail()
                    {
                        id = Guid.NewGuid(),
                        log_id = log.id,
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dg == DialogResult.OK)
                {
                    ...
                    var log = Log2.CreateLog(Log2.Log2Action.phantich, "khởi tạo dữ liệu đầu kỳ tháng " + string.Format("{0:MM/yyyy}", thang), "tbLuong_NhapDuLieu_Tax");
                    log.tbLog2_details.Add(new Core.Business.DbObject.tbLog2_detail()
                    {
                        id = Guid.NewGuid(),
                        log_id = log.id,
        ...
        {
             dlg = new dlgChuyenBangLuong_Thue();
            dlg.ShowDialog();
        }

        private void toolStripButton5_Click(object sender, EventArgs e)
        {
            dlg_BC = new dlgChuyenBangLuongBaoCao_Thue();
            dlg_BC.ShowDialog();
        }
        ...
        private void frmNhapThamSo_Tax_KeyDown(object sender, KeyEventArgs e)
        {
