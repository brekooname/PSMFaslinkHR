...
{
    public partial class dlgChuyenBangThue_Luong : DevExpress.XtraEditors.XtraForm
    {
      

        dcLuongv3DataContext db = new dcLuongv3DataContext(Core.Business.Provider.ConnectionString);

        public dlgChuyenBangThue_Luong()
        {
            InitializeComponent();
        }
        ...
        private void EvaluatorEditor_Load(object sender, EventArgs e)
        {
            ...
            txt_TsThue.Properties.DataSource = Provider.ExecuteDataTableReader("p_luong_tbLuong_DinhNghiaThamSo_Tax");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (IsLock.IsLock.Check_IsLock("tbLuong_NhapDuLieu", new DateTime(txt_ThangThue.DateTime.Year, txt_ThangThue.DateTime.Month, 1)))
            {
                GUIHelper.MessageBox("Dữ liệu lương đã chốt khổng thể thao tác!");
                return;
            }

            try
            {
                Provider.ExecuteDataTableReader("p_luong_layDuLieuThue_Luong",
                ...
                var log = Log2.CreateLog(Log2.Log2Action.phantich, "Chuyển tham số thuế sang tham số lương phòng ban  " + chonPhongBan1.SelectedRow.DepName, "tbLuong_NhapDuLieu");
                log.tbLog2_details.Add(new Core.Business.DbObject.tbLog2_detail()
                {
                    ...
                    " Tháng lương " + string.Format("{0:MM/yyyy}", new DateTime(txtThangLuong.DateTime.Year, txtThangLuong.DateTime.Month, 1)) +
                    " Tham số thuế " + txt_TsThue.EditValue + " tham số lương " + txt_TsLuong.EditValue + " phòng ban " + chonPhongBan1.SelectedRow.DepName
                });
                Log2.PushLog(log);

                GUIHelper.MessageBox("Chuyển tham số thuế sang tham số lương thành công");
