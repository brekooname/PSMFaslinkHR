...
namespace iHRM.Win.Frm.QuetThe
{
    public partial class frmDangKyLamThem : frmBase
    {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.Caption = "Đang tải dữ liệu đăng ký làm thêm...";
            dw_it.OnDoing = (s, ev) =>
            {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (IsLock.IsLock.Check_IsLock("tbDangKyCaLam1", item.ngay.Value))
                {
            ...
            var dg = MessageBox.Show("Bạn có chắc chắn muốn xóa tất cả đăng ký đang chọn?", "Xóa tất cả đăng ký đang chọn", MessageBoxButtons.OKCancel);
            if (dg == DialogResult.OK)
            {
                try
                {
                    ...
                    {
                        _lActionLog.Add(new ActionClass("Xóa dữ liệu", item.EmployeeID, "", string.Format("Xóa đăng ký làm thêm ngày {0:dd/MM/yyyy}, idCaLam ={1}", item.ngay, item.idCaLam), "tbDangKyCaLam"));
                    }
                    ...
                    foreach (tbDangKyCaLam item in listDKCaLam_NoLock)
                    {
                        ...
                        if (kqqt != null)
                        {
                            if (!IsLock.IsLock.Check_IsLock("tbKetQuaQuetThe", item.ngay.Value))
                            {
                                ...
                                _lActionLog.Add(new ActionClass("Xóa dữ liệu", item.EmployeeID, "", string.Format("Xóa dữ liệu kết quả quẹt thẻ ngày {0:dd/MM/yyyy}, EmployeeID ={1} do xóa đăng ký ca làm thêm", item.ngay, item.EmployeeID), "tbKetQuaQuetThe"));
                            }
                        }
                    }
                    db.SubmitChanges();
                    ...
                    {
                        GUIHelper.Notifications(string.Format("Có {0} bản ghi không được xóa vì đã chốt công.", lst.Count - listDKCaLam_NoLock.Count), "Xóa dữ liệu", GUIHelper.NotifiType.info);
                    }
                    else
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            ...
            {
                dlgDKCaLam = new dlgDangKyLamThem();
                dlgDKCaLam.ShowDialog();
            }
        ...
        {
            _lActionLog.Clear();
            var dg = MessageBox.Show("Bạn có chắc chắn muốn chốt dữ liệu đang chọn?", "Chốt đăng ký làm thêm", MessageBoxButtons.OKCancel);
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        //if (IsLock.IsLock.Check_IsLock("tbDangKyCaLam1", Convert.ToDateTime(r["ngay"].ToString())))
                        //{
                        //    GUIHelper.MessageBox("Dữ liệu đã chốt công khổng thể thao tác!");
                        //    continue;
                        //}

                        if (IsLock.IsLock.Check_IsLock_Type("tbDangKyCaLam1", Convert.ToDateTime(r["ngay"].ToString()),1))
                        {
                            GUIHelper.MessageBox("Dữ liệu đã chốt công khổng thể thao tác!");
                            continue;
                        }
                        ...
                        _lActionLog.Add(new ActionClass("Chốt đăng ký làm thêm", r["EmployeeID"].ToString(), "", string.Format("Chốt đăng ký làm thêm ngày {0:dd/MM/yyyy}", Convert.ToDateTime(r["ngay"])), "tbDangKyCaLam"));
                    }
                }
                LogAction.LogAction.PushLog(_lActionLog);
            }
        ...
        {
            var dg = MessageBox.Show("Bạn có chắc chắn muốn duyệt tất cả các đăng ký làm thêm?", "Duyệt đăng ký làm thêm", MessageBoxButtons.OKCancel);
            if (dg == DialogResult.OK)
            {
        ...
        {
            var dg = MessageBox.Show("Bạn có chắc chắn muốn gỡ chốt dữ liệu đang chọn?", "Gỡ chốt đăng ký làm thêm", MessageBoxButtons.OKCancel);
            if (dg == DialogResult.OK)
            {
                int[] rc = grv.GetSelectedRows();
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        _lActionLog.Add(new ActionClass("Gỡ chốt đăng ký làm thêm", r["EmployeeID"].ToString(), "", string.Format("Gỡ chốt đăng ký làm thêm ngày {0:dd/MM/yyyy}", Convert.ToDateTime(r["ngay"])), "tbDangKyCaLam"));
                    }
                }
                LogAction.LogAction.PushLog(_lActionLog);
            }
        ...
        {
            if (e.RowHandle >= 0 && grv.GetRowCellValue(e.RowHandle, "isAccept") != null && !string.IsNullOrEmpty(grv.GetRowCellValue(e.RowHandle, "isAccept").ToString()))
            {
        ...
        private void frmDangKyLamThem_KeyDown(object sender, KeyEventArgs e)
        {
