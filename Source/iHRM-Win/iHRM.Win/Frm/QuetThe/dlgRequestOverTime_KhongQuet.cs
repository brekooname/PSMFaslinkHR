...
{
    public partial class dlgRequestOverTime_KhongQuet : dlgCustomBase
    {
        ...
        public dlgRequestOverTime_KhongQuet()
        {
            InitializeComponent();
        }

        private void dlgRequestOverTime_KhongQuet_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void dlgRequestOverTime_KhongQuet_Load(object sender, EventArgs e)
        {
            ...
            this.Form_Title = Provider.ExecuteDataSetReader("p_Translate", new System.Data.SqlClient.SqlParameter("CtrName", "dlgRequestOverTime_KhongQuet_Title")).Tables[0].Rows[0][LoginHelper.langTrans].ToString();

            this.Form_Description = Provider.ExecuteDataSetReader("p_Translate", new System.Data.SqlClient.SqlParameter("CtrName", "dlgRequestOverTime_KhongQuet_Description")).Tables[0].Rows[0][LoginHelper.langTrans].ToString();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng chọn ngày đăng ký!", "Đăng ký tăng ca"
            ...
            {
                MessageBox.Show("Vui lòng nhập giờ đăng ký!", "Đăng ký tăng ca"
                                , MessageBoxButtons.OK
                                , MessageBoxIcon.Error);
            ...
            if (cbbLoaiDK.EditValue == "Ngoài kế hoạch" && btnFile.EditValue == null)
            {
                MessageBox.Show("Đăng ký ngoài kế hoạch. Vui lòng chọn file!", "Đăng ký tăng ca"
                                , MessageBoxButtons.OK
                                , MessageBoxIcon.Error);
            ...
            if (cbbLoaiDK.EditValue == null)
            {
                MessageBox.Show("Bạn chưa chọn loại đăng ký", "Đăng ký tăng ca"
                                , MessageBoxButtons.OK
                                , MessageBoxIcon.Error);
            ...
            dw_it.OnDoing = (s, ev) =>
            {
                ...
                foreach (string empID in _lEmp)
                {
                    ...
                    if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime_KhongQuet", _day, 1))
                    {
                        dw_it.bw.ReportProgress(2, string.Format("Dữ liệu đã chốt công ngày {0:dd/MM/yyyy} khổng thể thao tác!\r\n", _day));
                        continue;
                    }
                    if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime_KhongQuet", _day, 2))
                    {
                    ...
                    if(ii != null)
                    {
                        LogAction.LogAction.PushLog("Đăng ký tăng ca "
                            , empID
                            , ""
                            , string.Format("Đăng ký tăng ca từ giờ: {0} - đến giờ: {1} - ngày: {2:dd/MM/yyyy}"
                                            , txtTuGio.Text.ToString()
                                            , txtDenGio.Text.ToString()
                                            , _day)
                            , "tbRequestOverTime_KhongQuet");
        ...
        private void dlgRequestOverTime_KhongQuet_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control && e.Shift && e.KeyCode == Keys.T)
            {
                ...
                frm.ShowDialog();
                dlgRequestOverTime_KhongQuet_Load(null, null);
