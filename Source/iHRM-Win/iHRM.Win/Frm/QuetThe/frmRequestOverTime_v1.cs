...
{
    public partial class frmRequestOverTime_v1 : frmBase
    {
        ...
        dlgRequestOverTime_v1 dlgDKTangCa;

        bool isDuyet = false;

        public frmRequestOverTime_v1()
        {
            InitializeComponent();
        }
        ...
        {
            frmRequestOverTime_v1_Load(null, null);
        }
        //Test dữ liệu
  
       
        private void frmRequestOverTime_v1_Load(object sender, EventArgs e)
        {
                ...
                foreach (DataRow dr in dt.GetChanges().Rows)
                {
                    if (dr.RowState == DataRowState.Deleted)
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Xóa dữ liệu"
                            , empID
                            , ""
                            , "Xóa dữ liệu yêu cầu tăng ca từ " 
                            + tuGio.ToString() + " đến "
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Không duyệt chốt công"
                            , r["EmployeeID"].ToString()
                            , ""
                            , "Không duyệt chốt yêu cầu tăng ca "
                            + r["tuGio"].ToString() + " đến "
                            + r["denGio"].ToString() + " ngày "
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Gỡ duyệt"
                            , r["EmployeeID"].ToString()
                            , ""
                            , "Gỡ duyệt chốt công dữ liệu yêu cầu tăng ca "
                            + r["tuGio"].ToString() + " đến "
                            + r["denGio"].ToString() + " ngày "
                            + string.Format("{0: dd/MM/yyyy}", DateTime.Parse(r["ngay"].ToString()))
                            , "tbRequestOverTime"));
        ...
        private void toolStripThem_Click(object sender, EventArgs e)
        {
            if (dlgDKTangCa != null)
            {
                dlgDKTangCa.ShowDialog();
            }
            else
            {
                dlgDKTangCa = new dlgRequestOverTime_v1();
        ...
        private void frmRequestOverTime_v1_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (dt.GetChanges() == null || (dt.GetChanges() != null && dt.GetChanges().Rows.Count == 0))
            {
                return;
