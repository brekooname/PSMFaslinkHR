...
{
    public partial class frmRequestOverTime : frmBase
    {
        DataTable dt = new DataTable();
        dcDatabaseDataContext db = new dcDatabaseDataContext(Provider.ConnectionString);
        List<ActionClass> _lActionClass = new List<ActionClass>();
        bool isDuyet = false;
        public frmRequestOverTime()
        {
            InitializeComponent();
        }
        private void resItemButtonFile_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            else if (e.Button.Index == 1) // Chọn file mới
            {
                if (dr != null)
                {
                    ...
                    if (od.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Cập nhật", dr["EmployeeID"].ToString(), "", "Chọn file mới", "tbRequestOverTime"));
                    }
                }
                else
                {
                    GUIHelper.Notifications("Không có dữ liệu để chọn", "Chọn file", GUIHelper.NotifiType.error);
                }
            }
            else if (e.Button.Index == 2) // Xóa file
            {
                if (dr != null)
                {
                    ...
                    _lActionClass.Add(new ActionClass("Xóa dữ liệu", dr["EmployeeID"].ToString(), "", "Xóa file", "tbRequestOverTime"));
        ...
        private void toolStripSearch_Click(object sender, EventArgs e)
        {
            ...
            dt = Provider.ExecuteDataTable("p_getRequestOverTime_byStrEmp",
        ...
        private void toolStripLuu_Click(object sender, EventArgs e)
        {
            ...
            List<tbRequestOverTime> lst = new List<tbRequestOverTime>();
            try
            {
                foreach (DataRow dr in dt.GetChanges().Rows)
                {
                    if (dr.RowState != DataRowState.Deleted)
                    {
                        ...
                        try
                        {
                            tbRequestOverTime sc = new tbRequestOverTime();
                            sc.EmployeeID = dr["EmployeeID"].ToString();
                            if (dr["isAcceptBP"].ToString() == string.Empty && dr["isGoBo"].ToString() == string.Empty && dr.RowState == DataRowState.Added) //trạng thái đăng kí
                                sc.ghiChu = "2";
                        ...
                        {
                            _lActionClass.Add(new ActionClass("Thêm dữ liệu", dr["EmployeeID"].ToString(), "", 
                                "Thêm dữ liệu yêu cầu tăng ca từ " + dr["tuGio"].ToString() 
                                + " đến " + dr["denGio"].ToString() 
                                + " ngày " + string.Format("{0: dd/MM/yyyy}", Convert.ToDateTime(dr["ngay"])), "tbRequestOverTime"));
                        }
                        if (dr.RowState == DataRowState.Modified && isDuyet == false)
                        {
                            _lActionClass.Add(new ActionClass("Sửa dữ liệu", dr["EmployeeID"].ToString(), "", "Sửa dữ liệu yêu cầu tăng ca từ " + dr["tuGio"].ToString() 
                                + " đến " + dr["denGio"].ToString() 
                                + " ngày " + string.Format("{0: dd/MM/yyyy}", Convert.ToDateTime(dr["ngay"])), "tbRequestOverTime"));
                            isDuyet = false;
                        }
                    ...
                    else
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Xóa dữ liệu", empID, "", "Xóa dữ liệu yêu cầu tăng ca từ " + tuGio.ToString() + " đến " 
                            + denGio.ToString() + " ngày " 
                            + string.Format("{0: dd/MM/yyyy}", Ngay), "tbRequestOverTime"));
                    }

                }
                Provider.UpdateData(dt, "tbRequestOverTime");
                LogAction.LogAction.PushLog(_lActionClass); _lActionClass.Clear();
                #region mail
                try
                {
                    foreach (tbRequestOverTime l in lst)
                    {
                        db.SubmitChanges();
                        //tbRequestEditAttendane
                        tbRequestOverTime rq = db.tbRequestOverTimes.Where(p => p.EmployeeID == l.EmployeeID && p.ngay == l.ngay && p.ngayYeuCau == l.ngayYeuCau).FirstOrDefault();
                        if (rq == null)
                            continue;
                        w5sysUser u = db.w5sysUsers.Where(p => p.id == rq.idUserRequest).FirstOrDefault();
                        string NoiDung = string.Empty;
                        string TieuDe = "[THUANHAI - iHRM] - Đăng kí tăng ca- ";
                        ...
                        NoiDung += "Nội dung: Đăng ký tăng ca" + "\n";
                        if (l.ghiChu == null)
                            continue;
                        ...
                        NoiDung += "Thời gian: Ngày " + string.Format("{0: dd/MM/yyyy}", rq.ngay) + " từ giờ " 
                            + string.Format("{0:00}", l.tuGio == null ? rq.tuGio.Value.Hours : l.tuGio.Value.Hours) + ":" 
                            + string.Format("{0:00}", l.tuGio == null ? rq.tuGio.Value.Minutes : l.tuGio.Value.Minutes) + " tới giờ " 
                            + string.Format("{0:00}", l.denGio == null ? rq.denGio.Value.Hours : l.denGio.Value.Hours) + ":" 
                            + string.Format("{0:00}", l.denGio == null ? rq.denGio.Value.Minutes : l.denGio.Value.Minutes) + "\n";
        ...
        private void toolStripAccept_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        //if (IsLock.IsLock.Check_IsLock("tbRequestOverTime", Convert.ToDateTime(r["ngay"].ToString())))
                        //{
                        //    GUIHelper.MessageBox("Dữ liệu đã chốt công khổng thể thao tác!");
                        //    continue;
                        //}
                        if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime", Convert.ToDateTime(r["ngay"].ToString()), 1))
                        {
                            GUIHelper.MessageBox("Dữ liệu đã chốt công không thể thao tác!");
                            continue;
                        }
                        if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime", Convert.ToDateTime(r["ngay"].ToString()), 3))
                        {
                        ...
                        _lActionClass.Add(new ActionClass("Duyệt", r["EmployeeID"].ToString(), "", "Duyệt", "tbRequestOverTime"));
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        r["ngayAcceptBP"] = DateTime.Now;
                        r["ghiChuAcceptBP"] = frm.reason;
                        _lActionClass.Add(new ActionClass("Không duyệt", r["EmployeeID"].ToString(), "", "Không duyệt", "tbRequestOverTime"));
                ...
                for (int i = 0; i < grv.DataRowCount; i++)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        if (r["isAcceptBP"] == DBNull.Value)
                        {
                            ...
                            r["ngayAcceptBP"] = DateTime.Now;
                            r["ghiChuAcceptBP"] = frm.reason;
                            _lActionClass.Add(new ActionClass("Duyệt tất cả", r["EmployeeID"].ToString(), "", "Duyệt tất cả", "tbRequestOverTime"));
        ...
        private void grv_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            if (r != null && r.RowState != DataRowState.Unchanged)
            {
                ...
                if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime", Convert.ToDateTime(r["ngay"].ToString()), 2))
                {
            ...
            if (r != null && r.RowState == DataRowState.Unchanged)
            {
                ...
                if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime", ngay, 1))
                {
                    e.ErrorText = ("Dữ liệu đã chốt công khổng thể thao tác!\n");
                    e.Valid = false;
                    return;
                }
                if (IsLock.IsLock.Check_IsLock_Type("tbRequestOverTime", ngay, 2))
                {
                    e.ErrorText = ("Dữ liệu đã chốt đăng ký mới không thể thao tác!\n");
                    e.Valid = false;
                    return;
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        _lActionClass.Add(new ActionClass("Không duyệt chốt công", r["EmployeeID"].ToString(), "", "Không duyệt chốt công", "tbRequestOverTime"));
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        r["ngayAccept"] = DateTime.Now;
                        r["ghiChuAccept"] = frm.reason;
                        _lActionClass.Add(new ActionClass("Duyệt chốt công", r["EmployeeID"].ToString(), "", "Duyệt chốt công", "tbRequestOverTime"));
        ...
        private void toolStripButton_KhoaHan_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    _lActionClass.Add(new ActionClass("Khóa hạn", r["EmployeeID"].ToString(), "", "Khóa hạn", "tbRequestOverTime"));
            ...
            if (dg == DialogResult.OK)
            {
                ...
                for (int i = rc.Count(); i > 0; i--)
                {
                    ...
                    if (r != null)
                    {
                        ...
                        r["userGoBo"] = LoginHelper.user.id;
                        r["ngayGoBo"] = DateTime.Now;
                        _lActionClass.Add(new ActionClass("Gỡ duyệt", r["EmployeeID"].ToString(), "", "Gỡ duyệt chốt công dữ liệu yêu cầu tăng ca", "tbRequestOverTime"));
