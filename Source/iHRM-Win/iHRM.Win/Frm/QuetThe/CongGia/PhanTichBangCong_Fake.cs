...
{
    public partial class PhanTichBangCong_Fake : dlgCustomBase
    {
        DataTable dt = new DataTable();
        dsXuLyQuetThe ds = new dsXuLyQuetThe();

        dcDatabaseDataContext db = new dcDatabaseDataContext(Provider.ConnectionString);
        public PhanTichBangCong_Fake()
        {
            InitializeComponent();
        }
        ...
        {
            var rs = ds.tblEmpSalary.Where(p => p.EmployeeID == empID && p.DateChange >= chonKyLuong1.TuNgay && p.DateChange <= chonKyLuong1.TuNgay);
            if (rs.Count() > 0)
            {
        ...
        /// <returns></returns>
        private double tinhCongTheoLoai(dsXuLyQuetThe.tbKetQuaQuetThe_FakeRow kq, int type)
        {
            double result = 0;
            ...
            switch (type)
            {
                ...
                case 35: //35: Tính ra vào giữa ca. //Jun add thêm 26/06/2019		
        ...
        private void bgw_import_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (IsLock.IsLock.Check_IsLock("tbBangCongThang_Fake", chonKyLuong1.TuNgay) || IsLock.IsLock.Check_IsLock("tbBangCongThang_Fake", chonKyLuong1.DenNgay))
            {
            ...
            try
            {
                var dtEmp = Provider.ExecuteDataTableReader("p_getAllEmpIDtbKetQuaQuetThe_Fake", new System.Data.SqlClient.SqlParameter("tuNgay", chonKyLuong1.TuNgay), new System.Data.SqlClient.SqlParameter("denNgay", chonKyLuong1.DenNgay));
                ...
                Provider.LoadDataByProc(ds, "tbKetQuaQuetthe_Fake", "p_BangCongThang_Fake_getAllKQQT", new System.Data.SqlClient.SqlParameter("tuNgay", chonKyLuong1.TuNgay), new System.Data.SqlClient.SqlParameter("denNgay", chonKyLuong1.DenNgay));
                bgw_import.ReportProgress(2, string.Format("{0:HH:mm:ss}: Dữ liệu kết quả quẹt thẻ: {1:n0} records", DateTime.Now, ds.tbKetQuaQuetThe_Fake.Rows.Count));
                bgw_import.ReportProgress(2, string.Format("{0:HH:mm:ss}: Reset bảng công tháng: {1:n0} records", DateTime.Now, Provider.ExecNoneQuery("p_tinhCong_ResetBangCongThang_Fake", new System.Data.SqlClient.SqlParameter("thang", chonKyLuong1.Thang))));
                ...
                Provider.LoadDataBySql(ds, "tblRef_LoaiNgayLamThem", "SELECT * FROM tblRef_LoaiNgayLamThem");
                ...
                foreach (DataRow rEmp in dtEmp.Rows)
                {
                    ...
                    var _lKQ = ds.tbKetQuaQuetThe_Fake.Where(p => p.EmployeeID == empID && p.ngay >= chonKyLuong1.TuNgay && p.ngay <= chonKyLuong1.DenNgay);

                    var bc = ds.tbBangCongThang_Fake.Where(p => p.EmployeeID == empID && p.thang == thangXuLy).FirstOrDefault();

                    if (bc == null)
                    {
                        bc = ds.tbBangCongThang_Fake.NewRow() as dsXuLyQuetThe.tbBangCongThang_FakeRow;
                    ...
                    if (strAction == "addNew")
                    {
                        ds.tbBangCongThang_Fake.AddtbBangCongThang_FakeRow(bc);
                    }
                }

                bgw_import.ReportProgress(4, "Đang đẩy dữ liệu lên server");

                bgw_import.ReportProgress(2, string.Format("{0:HH:mm:ss}: Commit database: {1:n0} records", DateTime.Now, ds.tbBangCongThang_Fake.Rows.Count));

                Provider.UpdateData(ds, "tbBangCongThang_Fake");
                ...
                //{
                //    string empID = rEmp["EmployeeID"].ToString();
                //    //Ngày công tính thực tế
                //    Provider.ExecNoneQuery("p_tinhCong_NgayCongTinhLuongThucTe_Fake", new SqlParameter("empID", empID), new SqlParameter("thangXuLy", thangXuLy));
                //    //Ngày công tính lương VP
                //    Provider.ExecNoneQuery("p_tinhCong_NgayCongTinhLuongVP_Fake", new SqlParameter("empID", empID), new SqlParameter("thangXuLy", thangXuLy));
                //    //Ngày công tính lương CT
                //    Provider.ExecNoneQuery("p_tinhCong_NgayCongTinhLuongCT_Fake", new SqlParameter("empID", empID), new SqlParameter("thangXuLy", thangXuLy));
                //}

                bgw_import.ReportProgress(2, string.Format("{0:HH:mm:ss}: Phân tích bảng công thành công", DateTime.Now));

                LogAction.LogAction.PushLog("Analyze", "", "", string.Format("Phân tích bảng công tháng {0:dd/MM/yyyy}",thangXuLy), "tbBangCongThang_Fake");
            }
            catch (Exception ex)
            {
