...
{
    public partial class XuLyDuLieu_Fake : dlgCustomBase
    {
        Common.dlgChonDoiTuong chonDT = new Common.dlgChonDoiTuong();
        public XuLyDuLieu_Fake()
        {
            InitializeComponent();
        }
        private void XuLyDuLieu_Load(object sender, EventArgs e)
        {
            ...
            lblSoTieng.Text = SelectTranslate(lblSoTieng.Name, LoginHelper.langTrans);
            TranslateForm();
        }

        private void bgw_import_DoWork(object sender, DoWorkEventArgs e)
        {
            iHRM.Win.ExtClass.QuetThe_Fake.XuLyDuLieu controller = new iHRM.Win.ExtClass.QuetThe_Fake.XuLyDuLieu(LoginHelper.user);
            ...
            {
                i = i.AddDays(1);
                if (IsLock.IsLock.Check_IsLock("tbKetQuaQuetThe_Fake", i))
                {
            ...
            if (isContinue)
            {
                ...
                controller.doAnalyza(tuNgay,
                    ...
                    checkEdit1.Checked,
                    LoginHelper.user,
                    Convert.ToInt32(txtSoTieng.Text) //Số tiếng cắt tăng ca
                );

                LogAction.LogAction.PushLog("Analyze", ucChonDoiTuong_DS1.GetValue(), "", string.Format("Xử lý dữ liệu công BH từ ngày {0:dd/MM/yyyy} đến ngày {1:dd/MM/yyyy}", tuNgay, denNgay), "tbKetQuaQuetThe_Fake");
            }
        }
        ...
        private void btnDoImport_Click(object sender, EventArgs e)
        {
            //try
            //{
            //    //var a = Convert.ToInt32(txtSoTieng.Text);
            //}
            //catch (Exception)
            //{
            //    GUIHelper.MessageError("Bạn cần nhập số tiếng để cắt tăng ca trước khi xử lý.");
            //    return;
            //}
        ...
        private void btnCatQua4hTC_Click(object sender, EventArgs e)
        {
            try
            {
                var a = Convert.ToInt32(txtSoTieng.Text);
        ...
        private void XuLyDuLieu_Fake_KeyDown(object sender, KeyEventArgs e)
        {
