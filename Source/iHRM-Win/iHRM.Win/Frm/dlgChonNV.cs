...
{
    public partial class dlgChonNV : frmBase
    {
        ...
        {
            var lst = db.tblEmployees.Where(i => true);

            var a1 = db.tblRef_Departments.Where(p => p.DepID == LoginHelper.user.idKhoiPB.ToString()).FirstOrDefault();
            string PathOfStr = "";
            ...
            PathOfStr = a1.Path;
            lst = (from e1 in db.tblEmployees
                       join d in db.tblRef_Departments on e1.DepID_Final equals d.DepID
                   where d.Path.Contains(string.Format("/{0}/", LoginHelper.user.idKhoiPB)) && e1.LeftDate == null
                       select e1);
            ...
            grd.DataSource = lst.OrderBy(x => x.DepName_Final);
