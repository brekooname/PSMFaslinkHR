...
namespace iHRM.Win.Frm.Category
{
    public partial class Department : frmBase
    {
        ...
        {
            Data = CacheDataTable.GetCacheDataTable_Dep(TableConst.tblRef_Department.TableName);
        ...
        private void buttonPanel1_OnDelete(object sender, EventArgs e)
        {
            ...
            var dt = db.tblEmployees.SingleOrDefault(i => i.DepID_Final == DbHelper.DrGetString(CRow, TableConst.tblRef_Department.DepID));

            if (dt != null)
            {
                GUIHelper.MessageBox("Phòng ban đã được sử dụng. Không thể xóa. \n");
                return;
            }
        ...
        private void dlgEditor_OnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    // Trong update thay doi ben bang nhan vien 28/08/2017
                    //Begin
                    if (dep != null)
                    {
                        string DepID_Final = DbHelper.DrGetString(CRow, TableConst.tblRef_Department.DepID);

                        var emp = db.tblEmployees.Where(p => p.DepID_Final == DbHelper.DrGetString(CRow, TableConst.tblRef_Department.DepID));
                        
                        foreach (var r in emp)
                        {
                            r.DepID_Final = DepID_Final;

                            r.DepName_Final = db.tblRef_Departments.Where(p => p.DepID == DepID_Final).First().DepName;
                           
                            r.DepName = r.DepName_Final;
                        }
                    }
