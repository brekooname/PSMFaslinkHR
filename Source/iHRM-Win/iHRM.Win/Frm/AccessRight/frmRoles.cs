...
namespace iHRM.Win.Frm.AccessRight
{
    public partial class frmRoles : frmBase
    {
        ...
        private void buttonPanel2_OnSave(object sender, EventArgs e)
        {
            ...
            dw_it.OnDoing = (s, ev) =>
            {
                ...
                try
                {
                    ...
                    {
                        r1 = db.w5sysRoles.SingleOrDefault(i => i.id == (long)CRowRoles["id"]);
                        ...
                        db.w5sysRules.InsertAllOnSubmit(rules);//lỗi ở đây khi lưu xong. sửa nhóm khác or sửa tiếp khi lưu xong thì báo lỗi
                    ...
                    LogAction.LogAction.PushLog("Sửa dữ liệu", "", "", string.Format("Cập nhật quyền người dùng id={0}, caption={1}", r1.id, r1.caption), "w5sysRule");
        ...
        private void dlgEditor_OnSave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dlgEditor.CustomFormAction == 0)
                {
                    ...
                    LogAction.LogAction.PushLog("Thêm dữ liệu", "", "", string.Format("Thêm nhóm quyền id={0}, caption ={1}", r.id, r.caption), "w5sysRoles");
                ...
                else
                {
                    ...
                    LogAction.LogAction.PushLog("Sửa dữ liệu", "", "", string.Format("Sửa nhóm quyền id={0}, caption ={1}", r.id, r.caption), "w5sysRoles");
        ...
        private void buttonPanel1_OnDelete(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var ts = new System.Transactions.TransactionScope())
                {
                    ...
                    LogAction.LogAction.PushLog("Xóa dữ liệu", "", "", string.Format("Xóa nhóm quyền id={0}, caption ={1}", r1.id, r1.caption), "w5sysRoles");
                }
        ...
        dlgCustomRule frmCRule = null;
        private void repositoryItemButtonEdit1_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            var r = treeList1.GetDataRecordByNode(treeList1.FocusedNode) as Role.ruleAlias;
            
            if (r != null)
            {
                if (frmCRule == null)
                    
                    frmCRule = new dlgCustomRule();
                
                frmCRule.setDataSource(r.id);
                
                frmCRule.myRule = r.customRule;
               
                if (frmCRule.ShowDialog() == DialogResult.OK)
                {
                    treeList1.EditingValue = frmCRule.myRule;
                    
                    r.customRule = frmCRule.myRule;
                    
                    treeList1.FocusedColumn = treeListColumn1;
                    
                    System.Threading.Tasks.Task.Factory.StartNew(() 
                        => { 
                                System.Threading.Thread.Sleep(350); treeList1.FocusedColumn = treeListColumn3; 
