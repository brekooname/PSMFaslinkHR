...
namespace iHRM.Win.Frm
{
    ...
    {
        public static void setControlReadonly(Control c, bool readOnly)
        {
            if (c is DevExpress.XtraEditors.BaseEdit)
            {
                var cc = c as DevExpress.XtraEditors.BaseEdit;
                cc.Properties.ReadOnly = readOnly;
            }
            ...
            {
                foreach(var cc in (c as DevExpress.XtraGrid.GridControl).Views)
                {
                    if (cc is DevExpress.XtraGrid.Views.Grid.GridView)
                    {
                        var grv = cc as DevExpress.XtraGrid.Views.Grid.GridView;
                        grv.OptionsBehavior.ReadOnly = readOnly;
                        if (grv.OptionsBehavior.Editable)
                            grv.OptionsView.NewItemRowPosition = readOnly ? DevExpress.XtraGrid.Views.Grid.NewItemRowPosition.None : DevExpress.XtraGrid.Views.Grid.NewItemRowPosition.Bottom;
                    }
                }
            }

            foreach (Control cc in c.Controls)
                setControlReadonly(cc, readOnly);
        }
    }
}
