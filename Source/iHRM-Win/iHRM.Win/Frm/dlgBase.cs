...
{
    public partial class dlgBase : DevExpress.XtraEditors.XtraForm
    {
        public long _iRule = 0;
        public long iRule
        {
            get { return _iRule; }
            set { _iRule = value; buttonPanel1.setFunctionEnable((int)_iRule); }
        }
        ...
        protected virtual void FormSetData()
        {
            ...
            {
                if (it.Edit is TreeListLookUpEdit)
                {
                    var tree = it.Edit as TreeListLookUpEdit;
                    if (DbHelper.DrGet(myValue, it.DataIndex) != null)
                    {
                        it.Edit.EditValue = DbHelper.DrGet(myValue, it.DataIndex).ToString();
                    }
                    else
                    {
                        it.Edit.EditValue = null;
                    }
                }
                else
            ...
            foreach (var it in dlgData.CB)
            {
                if (myValue.Table.Columns.Contains(it.DataIndex))
                {
                    ...
                    else if (it.DataType == ControlBinding_DataType.Image)
                    {
                        ...
                        {
                            myValue[it.DataIndex] = DBNull.Value;
                        }
