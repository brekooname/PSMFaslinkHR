...
{
    public partial class BCSoGioTheoNgay : frmBase
    {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.OnDoing = (s,ev) => //hàm lấy dữ liệu chạy ngầm
            {
                ...
                {
                    dtData = logic.GetReportTotalHours(day, todates,ucChonDoiTuong_DS1.GetValue());
                    if (dtData.Rows.Count > 0)
                    {
                        dw_it.bw.ReportProgress(1, dtData.AsEnumerable().OrderBy(p => p["DepName"]).CopyToDataTable());
                    }
                    else
                    {
            ...
            dw_it.OnProcessing = (ps, dtData) => //hàm report //khi lấy đc dữ liệu sẽ đẩy về đây xử lý //có thể đẩy về nhiều lần từ doing
            {
                switch (dtData.ProgressPercentage)
                {
                    case 1:
                        grd.DataSource = dtData.UserState;
                        break;
                }
            };
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            ge.OnDoing += (bw) =>
            {
                bw.ReportProgress(-1, "Xuất excel thông tin nhân viên"); // Set caption
                ...
                ExcelExportHelper ex = new ExcelExportHelper("Report/ReportSoGioTheoNgay.xls"); // Tạo excel export helper + đường dẫn template
                ...
                ex.RendAndFlush("ReportSoGioTheoNgay_" + DbHelper.DrGetString(LoginHelper.Dept, "code"), sd.FileName);
                bw.ReportProgress(2);
                #endregion
            };
        ...
        #endregion
        private void BCSoGioTheoNgay_KeyDown(object sender, KeyEventArgs e)
        {
