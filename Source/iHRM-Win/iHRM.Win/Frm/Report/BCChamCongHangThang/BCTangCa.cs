...
{
    public partial class BCTangCa : frmBase
    {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            dw_it.OnDoing = (s, ev) => //hàm lấy dữ liệu chạy ngầm
            {
                try
                {
                    ...
                    data = Provider.ExecuteDataTable("p_BaoCao_KetQuaTangCa"
        ...
        private void btnExcelIn_Click(object sender, EventArgs e)
        {
            ...
            ge.OnDoing += (bw) =>
            {
                ...
                ExcelExportHelper ex = new ExcelExportHelper("Report/SW_HR_BAOCAOTANGCA_001.xlsx"); // Tạo excel export helper + đường dẫn template

                ex.WriteToCell("I5", chonKyLuong1.TuNgay.Date.ToString("dd/MM/yyyy") + " - " + chonKyLuong1.DenNgay.Date.ToString("dd/MM/yyyy"));

                ex.WriteToCell("E5", ucChonDoiTuong_DS1.getKhoiPBDisPlay());

                ex.FillDataTable(data); // Fill BC_FillData

                ex.RendAndFlush("BaoCaoKetQuaTangCa_" + DbHelper.DrGetString(LoginHelper.Dept, "code"), sd.FileName);
        ...
        #endregion
        private void BCTangCa_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control && e.Shift && e.KeyCode == Keys.T)
            {
                ...
                frm.ShowDialog();
                BCTangCa_Load(null, null);
            }
        }

        private void BCTangCa_Load(object sender, EventArgs e)
        {
            TranslateForm();
        }
    }
