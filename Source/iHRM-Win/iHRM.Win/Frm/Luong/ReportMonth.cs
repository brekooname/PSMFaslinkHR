...
namespace iHRM.Win.Frm.Luong
{
    public partial class ReportMonth : frmBase
    {
        ...
        private void toolStripExcelThue_Click(object sender, EventArgs e)
        {
            SaveFileDialog sd = new SaveFileDialog();
            sd.Filter = "Excel 2007|*.xls";
            if (sd.ShowDialog() != DialogResult.OK)
            ...
            ge.OnDoing += (bw) =>
            {
                ...
                {
                    var a = dtData.AsEnumerable().Where(p => p["ThueTNCN"] != null && Convert.ToDouble(p["ThueTNCN"]) > 0);
                    if (a.Count() > 0)
                    {
                        dtData = a.CopyToDataTable();
                    }
                    else
                    {
                        dtData = null;
                ...
                ExcelExportHelper ex = new ExcelExportHelper("Luong/baocaothue.xls");
                ex.WriteToCell("A2", db.w5Systems.Where(p => p.Ma == "TitlePhieuLuong").FirstOrDefault().KeywordVN.ToUpper());
                ex.WriteToCell("A4", string.Format("BÁO CÁO THUẾ THÁNG {0}/{1}", chonKyLuong1.DenNgay.Month, chonKyLuong1.DenNgay.Year));
                ex.WriteToCell("A5", "");
                ex.WriteToCell("A6", string.Format("Chu kỳ lương (Cycle Salary) : {0:dd/MM/yyyy} ~ {1:dd/MM/yyyy}", chonKyLuong1.TuNgay, chonKyLuong1.DenNgay));
                dtData.Columns.Add("tongTienTangCa_cn", typeof(double));
                foreach (DataRow dr in dtData.Rows)
                ...
                ex.FillDataTable(dtData);
                ex.RendAndFlush("baocaothue_" + DbHelper.DrGetString(LoginHelper.Dept, "code"), sd.FileName);
                bw.ReportProgress(2);
                #endregion
            };
        ...
        private void toolStripExcelBaoHiem_Click(object sender, EventArgs e)
        {
            SaveFileDialog sd = new SaveFileDialog();
            sd.Filter = "Excel 2007|*.xls";
            if (sd.ShowDialog() != DialogResult.OK)
            ...
            ge.OnDoing += (bw) =>
            {
                bw.ReportProgress(-1, "Xuất excel báo cáo bảo hiểm");
                ...
                {
                    var a = dtData.AsEnumerable().Where(p => p["tienBHXH"] != null && Convert.ToDouble(p["tienBHXH"]) > 0);
                    if (a.Count() > 0)
                    {
                        dtData = a.CopyToDataTable();
                    }
                ...
                ExcelExportHelper ex = new ExcelExportHelper("Luong/baohiem.xls");
                ex.WriteToCell("A2", db.w5Systems.Where(p => p.Ma == "TitlePhieuLuong").FirstOrDefault().KeywordVN.ToUpper());
                ex.WriteToCell("A4", string.Format("Báo cáo bảo hiểm tháng {0}/{1}", chonKyLuong1.DenNgay.Month, chonKyLuong1.DenNgay.Year));
                ex.WriteToCell("A5", "");
                ex.WriteToCell("A6", string.Format("Chu kỳ lương (Cycle Salary) : {0:dd/MM/yyyy} ~ {1:dd/MM/yyyy}", chonKyLuong1.TuNgay, chonKyLuong1.DenNgay));
                dtData.Columns.Add("tongTienTangCa_cn", typeof(double));
                foreach (DataRow dr in dtData.Rows)
                ...
                ex.FillDataTable(dtData);
                ex.RendAndFlush("baohiem_" + DbHelper.DrGetString(LoginHelper.Dept, "code"), sd.FileName);
                bw.ReportProgress(2);
                #endregion
            };
