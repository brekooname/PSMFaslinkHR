...
{
    public partial class frmSoNguoiPhuThuoc : frmBase
    {
        ...
        private void frmDangKyCaLam_Load(object sender, EventArgs e)
        {
            ...
            toolstripSEdit.Enabled = LoginHelper.user.isAdmin || BitHelper.Has(iRule.rules, (int)Enums.eFunction.Edit);
            ...
            var dtnv = Provider.ExecuteDataTableReader("p_getEmpByDepID_Name_NPT", new System.Data.SqlClient.SqlParameter("depID", LoginHelper.user.idKhoiPB));

            repositoryItemSearchLookUpEditMaNV.DataSource = dtnv;

            repositoryItemSearchLookUpEditMaNV.DisplayMember = "EmployeeName";

            repositoryItemSearchLookUpEditMaNV.ValueMember = "EmployeeName";

            //thông tin người phụ thuộc
            //var dt = Provider.ExecuteDataTableReader("p_getEmpByDepID_Name", new System.Data.SqlClient.SqlParameter("depID", 1));

            repositoryItemSearchLookUpEditNhanVien.DataSource = db.tblEmployees;
        ...
        private void grv_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            if (r != null && r.RowState != DataRowState.Unchanged)
            {
                ...
                //if (r["DateHetHan"] == DBNull.Value || r["DateHetHan"].ToString() == "")
                //{
                //    e.ErrorText = ("Chưa chọn ngày hết hạn.\n");

                //    e.Valid = false;

                //    grv.FocusedColumn = grv.Columns[10];

                //    return;
                //}
                //if (r["DateHetHan"] != DBNull.Value && DateTime.Parse(r["DateHetHan"].ToString()) < DateTime.Parse(r["DateHieuLuc"].ToString()))
                //{
                //    e.ErrorText = ("Ngày hết hạn không thể nhỏ hơn ngày có hiệu lực.\n");
